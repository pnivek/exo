...
def _patch_determine_available_memory() -> None:
    ...
    def patched(self: "Worker") -> int:
        ...
        # Let vLLM's native memory profiling run first — it accounts for model
        # weights, CUDA graphs, activation memory, and gpu_memory_utilization.
        vllm_result = 0
        real_empty_cache = torch.cuda.empty_cache
        torch.cuda.empty_cache = lambda: None  # type: ignore
        try:
            vllm_result = original(self)
            logger.info(f"vLLM native profiling allocated {vllm_result / (1024**3):.2f} GiB for KV cache")
        except (AssertionError, Exception) as e:
            logger.warning(f"vLLM native memory profiling failed: {e}")
        finally:
            torch.cuda.empty_cache = real_empty_cache  # type: ignore

        # Use vLLM's result if it's reasonable. Only override if it's clearly
        # too small (< what the model's max context needs).
        min_kv_bytes = 0
        try:
            ...
            logger.info(
                f"KV cache needed for max_model_len={max_seq_len}: "
        ...
        effective = max(vllm_result, min_kv_bytes)
        if effective <= 0:
            # Last resort fallback
            effective = int(os.environ.get("EXO_MIN_KV_CACHE_GIB", "10")) * 1024**3

        self._growable_max_kv_bytes = effective
        self.available_kv_cache_memory_bytes = effective
        logger.info(
            f"Growable KV cache: {effective / (1024**3):.2f} GiB "
            f"(vLLM profiled: {vllm_result / (1024**3):.2f} GiB, "
            f"model needs: {min_kv_bytes / (1024**3):.2f} GiB)"
        )
